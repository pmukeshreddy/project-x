...
def test_reference_projection_retains_exact_h_and_accounts_for_excluded_paths(store):
    ...
    projection=derive_reference(store,task_ref,runtime_policy())
...
def test_reference_projection_rejects_mismatched_classification_or_submission_policy(store,defect):
    ...
    with pytest.raises(QualificationRejected):derive_reference(store,task_ref,runtime_policy())
...
def test_reference_projection_handles_source_deletion_with_evidence(store):
    ...
    assert 'src/click/__init__.py' not in SubmissionService(store=store,policy=runtime_policy()).source(projection.projected_source).files
