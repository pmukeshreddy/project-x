...
Runner CPU/record limits and the remaining aggregate CPU boundary are detailed in `docs/reports/M7-runner.md`. The single retained Docker check uses an explicitly non-model, nontraining one-token scripted backend to respect the unchanged fixture contract; it is not evidence for native token sampling or task admission. The full native SFT/GRPO service remains subsequent work.
