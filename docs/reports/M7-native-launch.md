...
M3 remains the reviewed arm64 daemon through the explicitly configured protected Unix socket in the real M6 `CLIConfiguration.runtime`. See [the endpoint decision](../decisions/training-worker-endpoint.md). Forwarding that endpoint from a future CUDA controller remains unverified. Composition invokes actual M3 qualification; task resolution invokes current M5/M6 acceptance/revocation verification. A provisional TEST fixture does not satisfy these gates.

The smallest native **SFT mechanism smoke** uses one GPU, `groups_per_update=1`, `mini_batch_groups=1`, `TrainingConfig.max_updates=1`, `algorithm='sft'`, `group_size=4`, one frozen training seed and one actual released TRN task with an explicitly supplied verified source or trajectory demonstration. Use a context limit large enough for the exact full target and prefix, a frozen probe of 1–256 valid input tokens with 1–32 output tokens, and explicit `max_groups`/`max_wall_seconds`. The service checks nonzero gradient plus changed trainable and optimizer tensor content, then exact FSDP/controller RNG checkpoint reload and agreeing inference probes. These are implemented checks, not executed GPU evidence. A one-GPU fit is an external hardware/model gate, not a guarantee.

GRPO uses four distinct episode seeds and one explicit case seed per assigned group. The real feasibility gate permits at most64groups and requires8mixed groups over4training tasks before updates. Teacher solutions never replace that signal. Set the frozen roster, update/group/wall limits and initial/reference manifests explicitly. The current unknown-cost profile requires `budget_usd=null`; a finite currency cap is rejected without an actual native cost meter. This does not authorize paid resources. Wall checks occur between bounded phases; native lifecycle deadlines and sampled M3 CPU cleanup tolerances are not exact aggregate deadlines.

This direct library command is executable with real operator JSON files and the qualified environment. The M6 CLI wraps the same constructors. It does not provision anything or generate tasks. The five arguments are services JSON, NativeSettings JSON, TrainingConfig JSON, a JSON array of SourceDemonstration/TrajectoryDemonstration records, and a frozen invocation identifier:

```sh
/srv/feature-rl/skyrl/.venv/bin/python - services.json native.json training.json demonstrations.json smoke-sft-001 <<'PY'
import sys
...
from feature_rl.training.service import TrainingService, Demonstration
...
class Demonstrations:
    ...
    def model_validate_json(data):
        return TypeAdapter(tuple[Demonstration, ...]).validate_json(data)
demos = read_json(sys.argv[4], Demonstrations)
...
try:
    result = trainer.train(config, invocation=sys.argv[5], demonstrations=demos)
finally:
