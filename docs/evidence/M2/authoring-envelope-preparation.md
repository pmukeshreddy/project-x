...
### Evidence amendment: exact offline reproduction

The original 7,669-token figure was a sizing template, not a draft or production request. It included the license text as an `AuthoringContext(role="baseline")` only because `baseline` was the available non-request role in the measured interface. That label reproduced the original sizing calculation; it was not a decision that license text is baseline evidence and does not authorize semantic-role relabeling in production.

The coordinator resolved the production boundary after the original note: Click license text remains a deterministic eligibility and provenance input outside the model prompt, and M2 does not need a license-role extension. A single authorized offline tokenizer-only repeat reproduced the sizing template at 7,669 tokens and measured the corresponding production-shaped template without license text at **7,249 input tokens**. The latter request is 17,743 bytes (`ca2d064b70122c8b71795ed13f5113d40d9494d9f012e653f565c21362eac61b`), its canonical M0 output schema is 8,347 bytes (`ba71adaff7137ec03ccbbe081673f132038f24fc8880133f30c0e12216c8b4de`), and its provider prompt is 26,102 bytes (`3d379628fd02340466bf9911a071e4c61567363a11ec81eff8e982fdcdf14887`). Both measured envelopes exceed the currently qualified 2,048-token input profile.

The durable receipt is [`authoring-envelope/receipt.json`](authoring-envelope/receipt.json). It binds the exact command, environment overrides, UTC interval, exit status, raw stdout and stderr, source and span hashes, request/schema/prompt hashes, actual context roles, and every pinned tokenizer asset check. The reproducer is [`authoring-envelope/measure.py`](authoring-envelope/measure.py), with raw streams retained as [`measurement.stdout.json`](authoring-envelope/measurement.stdout.json) and [`measurement.stderr`](authoring-envelope/measurement.stderr). The repeat loaded only tokenizer/configuration assets from the already-pinned local directory. It made zero model, inference, network, or download calls.

The coordinator also fixed the later finalization boundary. Proposal fields and constraints must be derived from the actual M0 model definitions, while controller-owned envelope, provenance, and measured-cost metadata stays outside the proposal. The joined M2 service supplies that metadata after the provider call, validates the completed artifact with the actual M0 class, and enforces exact equality between a `ScenarioPlan`'s `mandatory_requirement_ids` and the frozen contract's mandatory feature plus compatibility IDs. M3 remains the sole product implementer until its reviewed runtime is handed off.

The synthetic M0 examples provide shape-only lower bounds: `RequirementContract` is 1,707 bytes / 429 tokens and `ScenarioPlan` is 1,583 bytes / 376 tokens. They contain one short requirement or scenario and are not Click drafts. A real contract with multiple evidence links, compatibility obligations and ambiguity decisions is provisionally estimated at 2,000–4,000 output tokens. Reviewed M3 discovery observations are likely to add roughly 2,048–4,096 input tokens, producing an estimated first contract input of about 9,717–11,765 tokens. These ranges are planning estimates. At the time of the original note, only the 7,669-token sizing envelope had been measured; the evidence amendment above adds the 7,249-token production-shaped measurement.
...
The evidence amendment records the subsequent coordinator decision to use that outside-prompt path for Click; no license-role extension is needed.
