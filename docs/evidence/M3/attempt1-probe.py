...
print('no_secrets',{f:p(f).exists() for f in ['/var/run/docker.sock','/Users','/root/.codex','/workspace/.git','/run/secrets']})
...
try:
 for _ in range(80):
  try:pid=os.fork()
  except OSError as e:print('fork_denied',e.errno,'children',len(children),flush=True);break
  if pid==0:time.sleep(20);os._exit(0)
  children.append(pid)
 print('pids_events',open('/sys/fs/cgroup/pids.events').read(),flush=True)
finally:
...
for directory in ['/workspace','/tmp']:
 n=0
 try:
  with open(directory+'/fill','wb',buffering=0) as f:
   while n<64*1024*1024:n+=f.write(b'x'*65536)
 except OSError as e:print('disk_denied',directory,e.errno,'bytes',n,flush=True)
 finally:os.unlink(directory+'/fill')
''')
probe('cpu',r'''
import time
print('before',open('/sys/fs/cgroup/cpu.stat').read(),flush=True)
a=time.monotonic();b=time.process_time()
while time.monotonic()-a<2:pass
print('cpu_seconds',time.process_time()-b,'wall_seconds',time.monotonic()-a,flush=True)
print('after',open('/sys/fs/cgroup/cpu.stat').read(),flush=True)
''')
probe('memory',r'''
print('allocating beyond cgroup 128MiB',flush=True)
a=bytearray(256*1024*1024)
print('UNEXPECTED_ALLOCATION',len(a),flush=True)
''')
probe('timeout',r'''
import os,time,pathlib
pathlib.Path('/workspace/interrupted').write_text('dirty')
pid=os.fork()
print('child',pid,'parent',os.getpid(),flush=True)
...
''',limit=16384)
probe('fresh_after_interruption',r'''
import pathlib,os
print('workspace_entries',list(pathlib.Path('/workspace').iterdir()))
print('tmp_entries',list(pathlib.Path('/tmp').iterdir()))
print('proc_pids',sorted(x.name for x in pathlib.Path('/proc').iterdir() if x.name.isdigit()))
''')
