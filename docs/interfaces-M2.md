...
Envelope content is re-serialized and validated through Pydantic's strict JSON transport. For schemas containing Literals, the provider first builds a side-effect-free validator from the pinned Pydantic core schema: it keeps the same unions, patterned mappings, references, objects, tuples, and containers, replaces model construction with inert validation models, removes user callbacks/default-factory execution, and wraps every Literal node with an exact raw-type check. The selected inert-model branch is then restored without constructors, defaults, or post-init hooks as that specific caller-model branch before the ordinary pinned validator runs, so a later union pass cannot silently choose a different Literal branch. This preserves M0-style tuples, enum-valued Literals and ordinary enums, and UTC timestamps. JSON strings matching string-enum Literal values remain valid; numeric equivalents such as `1` for `Literal[True]` or `3.0` for `Literal[3]` reject. A Literal schema containing a plain custom validator, custom model initializer, dataclass, call core form, or custom validator inside a union alternative is explicitly unsupported and fails with an attributable `GenerationSchemaUnsupportedError` before backend verification or execution.
