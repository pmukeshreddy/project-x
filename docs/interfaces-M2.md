...
Envelope content is re-serialized and validated through Pydantic's strict JSON transport. For schemas containing Literals, the provider inventories every pinned core-schema form, including definitions, before execution. The exact validator keeps supported unions, patterned mappings, references, objects, tuples and containers, replaces model construction with inert validation models, and wraps every Literal node with an exact raw-JSON-type check. A schema made only from supported ordinary core forms returns that single validator's selected result; there is no second branch-selection pass, so scalar alternatives such as raw integer `1` in `Literal[True] | int`, model alternatives and nested containers retain their selected Python and JSON types. Schemas with an after validator, default or post-init hook and no union run the ordinary caller validator once on the original canonical JSON after exact prevalidation. This preserves JSON callback mode, the actual M0 tuple/enum/UTC transport and simple lifecycle behavior.

The provider explicitly refuses Literal schemas whose semantics the inert graph cannot preserve: before, wrap or plain validators; custom initialization; dataclass or call forms; unknown core-schema forms; and any union combined with after callbacks, defaults or post-init lifecycle hooks. These forms raise an attributable `GenerationSchemaUnsupportedError` before backend verification or execution, and publication failure remains replayable without execution. JSON strings matching string-enum Literal values remain valid; numeric equivalents such as `1` for `Literal[True]` or `3.0` for `Literal[3]` reject when they have no valid exact numeric alternative. Actual M0 `RequirementContract` and `ScenarioPlan` JSON fixtures pass this supported boundary. A future caller needing a refused form must use a supported proposal schema or extend and review this boundary before the call.
