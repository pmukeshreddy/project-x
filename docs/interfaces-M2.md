...
Extra or duplicate JSON keys, malformed content, a changed response ID, missing/reordered/unknown source IDs, unknown or duplicate requirement IDs, tool events, event-order drift, model mismatch, missing/nonfinite selected-model logprobs, inconsistent usage, nonzero exits, cleanup failure, and resource termination all raise `GenerationProviderError`. The exception contains the immutable failure `GenerationCallRecord`.
...
| process physical footprint | external `proc_pid_rusage` sampling every 20 ms; process group killed above 4 GiB |
| declared memory ceiling | 5 GiB, leaving a 1 GiB guard band above the sampled kill threshold |
| process cleanup | fresh process group is killed on a boundary failure and absence is checked after exit |
...
Every success and failure writes separate opaque byte artifacts named `generation-request`, `generation-response`, `generation-retrieval`, `generation-schema`, `generation-options`, `generation-provenance`, `generation-usage`, `generation-cost`, `generation-events`, and `generation-status`. The response receipt records exit status, termination, wall/CPU measurements, sample count, maximum sampled and lifetime memory observations, any breach sample, stderr/stdout, and cleanup result. The raw JSONL event stream is retained byte-for-byte. On a failed, truncated, or malformed response, the usage archive separately retains every internally consistent accepted-input count/ID sequence and emitted token/model-score pair observed before failure, labels the response usage unaccepted, and never upgrades those partial observations to a successful `GenerationUsage`.
