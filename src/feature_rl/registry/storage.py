...
from .models import RegistryEvent, RegistryIntegrityError, RegistryIOError, RegistryLimit, RegistryConflict, RecoveryReport
...
class Storage:
    def __init__(self, root, limits, store_root):
        ...
        self.limits = limits
        self.configuration = canonical_json({'version': 1, 'limits': document(limits), 'store_root': str(store_root.absolute())})
    ...
    def shape(self, directory):
        ...
        for name in names:
            ...
            cap = self.limits.max_journal_bytes if name == 'events.jsonl' else self.limits.max_database_bytes
            if meta.st_size > cap:
    ...
    def recovered(self, con, directory):
        con.execute('BEGIN IMMEDIATE')
    ...
    def recover(self):
        with self.session() as (con, directory):
            return self.recovered(con, directory)[3]

    def view(self, reader):
        with self.session() as (con, directory):
            state, events, _, _ = self.recovered(con, directory)
    ...
    def change(self, builder):
        with self.session() as (con, directory):
            ...
            action, key, data = builder(state)
            prior = next((e for e in events if e.semantic_key == key), None)
            if prior is not None:
                if prior.action != action or canonical_json(prior.data) != canonical_json(data):
