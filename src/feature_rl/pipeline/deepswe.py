...
def add_cli(commands):
    parser = commands.add_parser('deepswe', help='import, run, and GRPO-train official DeepSWE environments')
    ...
    for action in ('exec','reset','close','collect'):
        ...
        if action == 'collect':
            command.add_argument('--output', required=True)
    train = actions.add_parser('train', help='one packaged task through the existing SkyRL GRPO update')
